...
$(document).ready(function () {
  ...
  $("#delete-many-btn").click(function () {
    ...
    if (confirm("Xóa " + values.length + " người dùng đã chọn??"))
  ...
  $("body").on("click", ".delete", function () {
    let delId = $(this).val();
    if (confirm("Xóa user sẽ xóa thông tin và orders của user???")) {
...
function callAjax(pageNo) {
  $.ajax({
    url: domain + "/admin/management-user-ajax",
    data: {
      pageNo: pageNo,
...
// lấy id role filter
function getRoleId() {
  return Number.parseInt($("#roleId").val());
}

// lấy id status filter
function getStatus() {
  return Number.parseInt($("#status").val());
}
